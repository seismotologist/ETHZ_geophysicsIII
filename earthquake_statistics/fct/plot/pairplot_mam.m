function pairplot_mam(meas, label, group, colors, mode)
...
for i = 1:size(meas, 2)
	for j = 1:size(meas, 2)
		...
		if i==j
			...
			for g = 1:ngroup
                x = meas(strcmp(group, groupName{g}), i);
                y = meas(strcmp(group, groupName{g}), j);
				plot(x, y, '.', 'Color', colors(g,:));
                
                rr   = corrcoef(x,y);
                xlm  = get(gca,'xlim');
                ylm  = get(gca,'ylim');
                xtxt = xlm(1)+3/4*(xlm(2)-xlm(1));
                ytxt = ylm(1)+ .1*(ylm(2)-ylm(1));
                text(xtxt,ytxt,sprintf('r%4.2f',rr(1,2)),'fontSize',8)
			end	
